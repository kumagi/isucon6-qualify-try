...
"""
    ...
    cur2 = dbh().cursor()
    cur2.execute('SELECT keyword FROM entry ORDER BY CHARACTER_LENGTH(keyword) DESC')
    keywords = cur2.fetchall()
    keyword_re = re.compile("(%s)" % '|'.join([re.escape(k['keyword']) for k in keywords]))
    cache_keyword_re = keyword_re
...
def load_stars(keyword):
    ...
    if key == None or key == "":
      return key
    cur = dbh().cursor()
    cur.execute('SELECT * FROM star WHERE keyword = %s', (key, ))
